			...
			<div class="side-nav-menu">
				...
				<div class="side-nav-button">
					<a href="message.php">
						...
						<?php
							if(validSession()){
								$requete = $pdo->prepare("SELECT COUNT(*) FROM message WHERE receiver = ? AND view = 0");
								$requete->execute(array($_SESSION['id']));
								$messageData = $requete->fetchAll();

								if($messageData[0][0] > 0){
									echo '<div class="side-nav-message-notification"></div>';
								}
							}
						?>
				...
				<div class="side-nav-button" onclick="openNotificationSide()">
					<a>
						...
						<?php
							if(validSession()){
								$requete = $pdo->prepare("SELECT COUNT(*) FROM notification WHERE notified = ? AND view = 0");
								$requete->execute(array($_SESSION['id']));
								$notificationData = $requete->fetchAll();

								if($notificationData[0][0] > 0){
									echo '<div class="side-nav-notification-notification"></div>';
								}
							}
						?>
				...
				<?php
					if(validSession()){
						echo '<div class="side-nav-button">
							<a href="profile.php?profile='.$_SESSION['pseudo'].'">
								<img class="side-nav-profile" src="'.$_SESSION['profile'].'" />
								<span>Profil</span>
							</a>
						</div>';
							...
							<div class="side-nav-button">
								<a href="admin.php">';
										if( basename($_SERVER['PHP_SELF']) != "admin.php"){
											echo '
											<svg height="24" width="24" viewBox="0 0 24 24">
												<polyline fill="none" points="16.723 8.93 10.498 15.155 7.277 11.933" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.978"/>
												<path d="M3 13.5a9 9 0 0 0 18 0V4.488A17.848 17.848 0 0 1 12 1.5a17.848 17.848 0 0 1-9 2.988Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.978"/>
											</svg>';
										}
										else{
											echo '
											<svg height="24" width="24" viewBox="0 0 24 24">
												<path d="M3 13.5a9 9 0 0 0 18 0V4.488A17.848 17.848 0 0 1 12 1.5a17.848 17.848 0 0 1-9 2.988Z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.978"/>
												<polyline points="16.723 8.93 10.498 15.155 7.277 11.933" stroke="rgb(255, 255, 255)" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.978"/>
											</svg>';
										}
							echo '
									<span>Admin</span>
						...
						echo '
						<div class="side-nav-button side-nav-button-bottom">
							<a id="side-nav-sub-menu">
								<svg height="24" width="24" viewBox="0 0 24 24" >
						...
						<div class="side-nav-sub-menu">
							...
							<div class="side-nav-sub-button" onclick="toggleDarkMode();">
								<svg height="18" width="18" viewBox="0 0 24 24">
								...
								<span>Changer l\'apparence</span>
							</div>
							<div class="side-nav-sub-button">
								<a href="php/session.php?disconnect=1">
